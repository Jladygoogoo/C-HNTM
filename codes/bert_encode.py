'''
source: https://github.com/adalmia96/Cluster-Analysis
'''
...
argparser.add_argument('--agg_by', default="average", type=str, help="method for aggregating compound words")
argparser.add_argument('--device', default=0, required=False)
argparser.add_argument('--docs_path', required=True, type=str)
argparser.add_argument('--dict_path', required=True, type=str)
argparser.add_argument('--data', default="20News", required=False)
...
class BertWordFromTextEncoder:
    ...
    def eb_dump(self, save_fn):
        ...
        all_vecs = []
        miss_count = 0
        for word in self.valid_vocab:
            #word = word.encode('utf-8', 'ignore').decode('utf-8')
            if word in self.w2vb:
                vec = np.around(self.w2vb[word]/self.w2vc[word], 8)
    ...
    def encode_docs(self, docs=[], agg_by="firstword", save_fn="", layer=12):
        ...
        if len(save_fn)==0:
            save_fn = f"{args.data}-bert-layer{args.nlayer}-{agg_by}.pkl"
...
def main():
    with open(args.docs_path, 'rb') as f:
        docs = pickle.load(f)
    # files = open(args.docs_path).read().splitlines()
    with open(args.dict_path, 'rb') as f:
        valid_vocab = list(pickle.load(f).token2id.keys())

    ### this is what you care about
    encoder = BertWordFromTextEncoder(valid_vocab=valid_vocab)
    encoder.test_encoder()
    encoder.encode_docs(docs=docs, save_fn=args.save_fn, agg_by=args.agg_by, layer=args.nlayer)    
